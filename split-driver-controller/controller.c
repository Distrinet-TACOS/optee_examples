...
static int update_buffers(struct update_buffer *buffers)
{
	struct update_buffer *buf;
	struct split_device *device;

	for (buf = buffers; buf->next != NULL; buf = buf->next) {
...
static int notification_handler(void)
{
	...
	size_t mem_size = no_devices * sizeof(struct update_buffer);
	...
	mem = tee_shm_alloc(dev_data.ctx, mem_size, flags);
	if (IS_ERR(mem)) {
...
static int open_session(dev_t dev, u32 *sess_id)
{
	...
	if ((ret < 0) || (sess_arg.ret != 0)) {
		pr_err("tee_client_open_session failed for device %u, err: %x\n",
		       dev, sess_arg.ret);
		return -EINVAL;
	}
...
{
	tee_client_close_session(dev_data.ctx, sess_id);
	return 0;
}
...
{
	create_context();
	open_session(dev_data.dev, &dev_data.sess_id);
	enable_notification();
...
static void __exit controller_exit(void)
{
	...
	close_session(dev_data.sess_id);
	destroy_context();
}
...
int register_split_driver(dev_t dev, const char name, void (*update)(void))
{
	...
	device = kcalloc(sizeof(struct split_device), GFP_KERNEL);
	device->dev = dev;
	device->name = name;
	...
	list_add(&device, &split_devices);
	no_devices++;

	return 0;
}
EXPORT_SYMBOL_GPL(register_split_driver);
...
EXPORT_SYMBOL_GPL(unregister_split_driver);
...
int open_optee_session(dev_t dev)
{
	...
	list_for_each_entry (device, &split_devices, list) {
		if (device->dev == dev) {
			open_session(dev, &device->sess_id);
...
EXPORT_SYMBOL_GPL(open_optee_session);
...
int close_optee_session(dev_t dev)
{
	...
	list_for_each_entry (device, &split_devices, list) {
		if (device->dev == dev & device->sess_id > 0) {
			...
			ret = tee_client_close_session(&dev_data.ctx,
						       device->sess_id);
			return 0;
		}
	}
...
EXPORT_SYMBOL_GPL(close_optee_session);
	...
	list_for_each_entry (device, &split_devices, list) {
		if (device->dev == dev && device->sess_id > 0) {
			*count = kfifo_len(&device->buffer);
			...
			kfifo_out(&device->buffer, buf, *count);
			return 0;
		}
	}
